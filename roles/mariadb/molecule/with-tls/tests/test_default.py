...
def test_certificate_directory(host, get_vars):
    ...
    directories = [
        os.path.dirname(
            config.get("ssl-cert", "/etc/mysql/certificates/molecule.lan.pem")
        ),
        os.path.dirname(
            config.get("ssl-key", "/etc/mysql/certificates/molecule.lan.key")
        ),
    ]
...
def test_certificates(host, get_vars):
    ...
    """
    files = [
        get_vars.get("mariadb_config_client", {}).get(
            "ssl-cert", "/etc/mysql/certificates"
        ),
        get_vars.get("mariadb_config_client", {}).get(
            "ssl-key", "/etc/mysql/certificates"
        ),
    ]
